<div id="header" style="display: inline-block; width: 100%">
    <#if tickets?has_content>
        <h2 style="float: left;">Please, see bellow list of booked tickets for event ${event.name}:</h2>
    <#else>
        <h2 style="float: left;">There is no booked tickets for event ${event.name}</h2>
    </#if>
